# Hi, I'm Chirag Garg 👋

### Full-Stack Developer | React • Next.js • Node.js • TypeScript • Python

I'm a Computer Science graduate and Full-Stack Developer focused on building
modern, scalable web applications, AI-powered tools, and SaaS products.

I enjoy turning ideas into production-ready applications with clean,
maintainable code and intuitive user experiences.

---

## 🚀 What I Build

- 🌐 Full-Stack Web Applications
- 🤖 AI-Powered Applications
- 💼 SaaS Platforms
- 📊 Data & Personal Finance Applications
- 🔐 Secure REST APIs & Authentication Systems
- 📱 Responsive Front-End Applications

---

## 🛠️ Tech Stack

### Languages
![JavaScript](https://img.shields.io/badge/JavaScript-000?style=flat-square&logo=javascript)
![TypeScript](https://img.shields.io/badge/TypeScript-000?style=flat-square&logo=typescript)
![Python](https://img.shields.io/badge/Python-000?style=flat-square&logo=python)
![Java](https://img.shields.io/badge/Java-000?style=flat-square&logo=openjdk)
![C++](https://img.shields.io/badge/C%2B%2B-000?style=flat-square&logo=cplusplus)

### Frontend
![React](https://img.shields.io/badge/React-000?style=flat-square&logo=react)
![Next.js](https://img.shields.io/badge/Next.js-000?style=flat-square&logo=nextdotjs)
![HTML5](https://img.shields.io/badge/HTML5-000?style=flat-square&logo=html5)
![CSS3](https://img.shields.io/badge/CSS3-000?style=flat-square&logo=css3)
![Tailwind CSS](https://img.shields.io/badge/Tailwind_CSS-000?style=flat-square&logo=tailwindcss)

### Backend
![Node.js](https://img.shields.io/badge/Node.js-000?style=flat-square&logo=nodedotjs)
![Express](https://img.shields.io/badge/Express-000?style=flat-square&logo=express)
![FastAPI](https://img.shields.io/badge/FastAPI-000?style=flat-square&logo=fastapi)

### Databases
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-000?style=flat-square&logo=postgresql)
![MongoDB](https://img.shields.io/badge/MongoDB-000?style=flat-square&logo=mongodb)
![MySQL](https://img.shields.io/badge/MySQL-000?style=flat-square&logo=mysql)
![SQLite](https://img.shields.io/badge/SQLite-000?style=flat-square&logo=sqlite)

### Cloud & Tools
![AWS](https://img.shields.io/badge/AWS-000?style=flat-square&logo=amazonaws)
![Docker](https://img.shields.io/badge/Docker-000?style=flat-square&logo=docker)
![Git](https://img.shields.io/badge/Git-000?style=flat-square&logo=git)
![GitHub](https://img.shields.io/badge/GitHub-000?style=flat-square&logo=github)

---

## ⭐ Featured Projects

### 🏗️ TaskForge
**Multi-tenant B2B task management SaaS**

A full-stack SaaS platform designed for teams to manage tasks,
projects, users, and organizations.

**Tech:** React • FastAPI • Clerk • SQLite

---

### 💰 Finora — Personal Finance
**AI-powered personal finance platform**

A modern finance application for tracking transactions, managing budgets,
analyzing spending, and using AI to understand financial data.

**Tech:** Next.js • Prisma • PostgreSQL • Clerk • Gemini

---

### 🤖 AI Resume Analyzer
**AI-powered resume analysis platform**

Analyzes resumes for ATS compatibility, provides scoring, and generates
personalized feedback to help improve job applications.

**Tech:** JavaScript • AI • Resume Analysis

---

### 🚗 Car Rental System
**Full-stack car rental platform**

A web application for browsing vehicles, managing rentals, and processing
online payments.

**Tech:** React • Node.js • Express • MongoDB • Stripe

---

### 🌐 Developer Portfolio
**Personal developer portfolio**

Responsive portfolio showcasing projects, technical skills, and experience.

**Tech:** React • Vite • Tailwind CSS • Three.js

---

## 📈 Currently Learning

- Advanced TypeScript
- System Design
- Backend Architecture
- Cloud & AWS
- AI/LLM Application Development
- Data Structures & Algorithms

---

## 🎯 Career Goals

I'm currently focused on opportunities in:

**Junior Software Development • Full-Stack Development • Frontend Development • Backend Development**

---

## 🤝 Let's Connect

📍 Toronto, Ontario, Canada

💼 [LinkedIn](https://www.linkedin.com/in/cgarg3)

🌐 Portfolio: **Coming Soon**

---

⭐ Feel free to explore my repositories and projects.
